...
public class SeatReservationApp {
	...
	static void menu(int cnt, int userinput) {
	...
	static void reservation(Seat[] seats) {
		System.out.println("\n 예매할 좌석을 선택해주세요");
		int input;
	...
	static void cancle(Seat[] seats) {
		System.out.println("예매를 취소할 좌석을 입력해주세요 ");
		int userinput = sc.nextInt() - 1;
		// 이미 예약이 가능하다면
		if (seats[userinput].isReservation == false) {
	...
	static void ifFullcancleAndClose(Seat[] seats) {
